...
export async function GET(request, { params }) {
  try {
    ...
    const project = await ShopifyProject.findOne({
      $or: [{ customId: id }, { _id: id.match(/^[0-9a-fA-F]{24}$/) ? id : null }],
    });

    if (!project) {
      return NextResponse.json(
        { success: false, message: 'Shopify project not found' },
        { status: 404 }
      );
    }
...
export async function PUT(request, { params }) {
  try {
    ...
    const updated = await ShopifyProject.findOneAndUpdate(
      {
        $or: [{ customId: id }, { _id: id.match(/^[0-9a-fA-F]{24}$/) ? id : null }],
      },
      {
...
export async function DELETE(request, { params }) {
  try {
    ...
    const deleted = await ShopifyProject.findOneAndDelete({
      $or: [{ customId: id }, { _id: id.match(/^[0-9a-fA-F]{24}$/) ? id : null }],
    });
